...
def t_n_minus_1(answers, n, t):
    # t/(n-1)-алгоритм: выбирает результат из n версий
    # Подсчёт групп совпадений
    answer_counts = Counter(answers)
    # Шаг 1: Группа с >t совпадений
    for answer, count in answer_counts.most_common():
        if count > t:
            return answer
    # Шаг 2: Группа с t совпадений, если t = (n-1)/2
    if t == (n - 1) // 2:
        for answer, count in answer_counts.items():
            if count == t:
                return answer
    # Шаг 3: Если есть расхождения (несколько групп), берём последний ответ
    return answers[-1] if len(answer_counts) > 1 else list(answer_counts.keys())[0]
...
def process_module(db_path, module_name, experiment_names, n, t, version_id_map):
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        exp_stats = {exp: {'matches': 0, 'total': 0} for exp in experiment_names if
                     exp.startswith(f'M{module_name[6:]}')}
